...
from utils import encode_batch, get_device, load_config, load_model_and_tokenizer, setup_logging
...
def main(args):
    ...
    if len(metadata) != index.ntotal:
        logger.warning("Warning: metadata count and index size differ.")
    ...
    while True:
        ...
        for r in results:
            logger.info(
                f"  Rank {r['rank']}: score={r['score']:.4f}, doc_id={r['doc_id']}, title={r['title']}"
            )
